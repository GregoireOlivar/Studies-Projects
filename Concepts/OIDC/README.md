I did the scheme on Draw.io and these are the explanations : 

1. User Initiates Login : The user clicks the /login button on the client application's web page and it's gonna trigger the OIDC flow. 
...
Once validated, the client extracts the user identifiers like the sub (Subject) and requested scopes (e.g., email). Based on the sub, the application will:

Identify and log in an existing user account in its own database.

Or create a new user account on the fly if this is their first time connecting ("just-in-time" provisioning).

Establish Application Session
Now that the user is fully authenticated, the client application issues its own local application session. It typically sets a secure, encrypted session cookie in the user's browser. Future requests from the user's browser to the application will include this cookie, allowing them to remain logged into the website without needing to go through the OIDC flow again.